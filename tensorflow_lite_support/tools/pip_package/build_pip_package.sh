...
function reorganize_includes() {
  TMPDIR="${1%/}"
  # pushd "${TMPDIR}/tflite_support/include/"
...
function prepare_src() {
  ...
  if is_windows; then
    rm -rf ./bazel-bin/tensorflow_lite_support/tools/pip_package/simple_console_for_window_unzip
    mkdir -p ./bazel-bin/tensorflow_lite_support/tools/pip_package/simple_console_for_window_unzip
    echo "Unzipping simple_console_for_windows.zip to create runfiles tree..."
    unzip -o -q ./bazel-bin/tensorflow_lite_support/tools/pip_package/simple_console_for_windows.zip -d ./bazel-bin/tensorflow_lite_support/tools/pip_package/simple_console_for_window_unzip
    echo "Unzip finished."
    # runfiles structure after unzip the python binary
    cp \
      bazel-bin/tensorflow_lite_support/tools/pip_package/simple_console_for_window_unzip/runfiles/org_tensorflow_lite_support/LICENSE \
      "${TMPDIR}"
    cp -R \
      bazel-bin/tensorflow_lite_support/tools/pip_package/simple_console_for_window_unzip/runfiles/org_tensorflow_lite_support/tensorflow_lite_support \
      "${TMPDIR}"
    RUNFILES=bazel-bin/tensorflow_lite_support/tools/pip_package/simple_console_for_window_unzip/runfiles/org_tensorflow_lite_support
  else
    RUNFILES=bazel-bin/tensorflow_lite_support/tools/pip_package/build_pip_package.runfiles/org_tensorflow_lite_support
    if [ -d bazel-bin/tensorflow_lite_support/tools/pip_package/build_pip_package.runfiles/org_tensorflow_lite_support/external ]; then
      # Old-style runfiles structure (--legacy_external_runfiles).
      cp \
        bazel-bin/tensorflow_lite_support/tools/pip_package/build_pip_package.runfiles/org_tensorflow_lite_support/LICENSE \
        "${TMPDIR}"
      cp -R \
        bazel-bin/tensorflow_lite_support/tools/pip_package/build_pip_package.runfiles/org_tensorflow_lite_support/tensorflow_lite_support \
        "${TMPDIR}"
    else
